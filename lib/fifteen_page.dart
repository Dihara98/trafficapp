...
class _FifteenPageState extends State<FifteenPage> {
  ...
  Map<String, dynamic>? driverDetails;
  Map<String, dynamic>? vehicleDetails;
  Map<String, dynamic>? officerDetails; // Declare officerDetails as a Map
  String? errorMessage;
  ...
  Future<void> _fetchOfficerDetails() async {
    try {
      // Get the officerId based on the logged-in user
      String officerId = FirebaseAuth.instance.currentUser!.uid;
      QuerySnapshot officerSnapshot = await FirebaseFirestore.instance
      ...
      if (officerSnapshot.docs.isNotEmpty) {
        setState(() {
          officerDetails = officerSnapshot.docs.first.data() as Map<String, dynamic>?;
          errorMessage = null;
        });
      } else {
        setState(() {
          officerDetails = null;
          errorMessage = 'Error retrieving officer details.';
        });
      }
    } catch (e) {
      setState(() {
        errorMessage = 'Error retrieving officer details: $e';
        officerDetails = null;
      });
    }
  }
  ...
  void _goToNextPage() {
    ...
    // Check if officerDetails is null
    if (officerDetails == null) {
      setState(() {
        _errorMessage = 'Error retrieving officer details. Please try again.';
      });
      return; // Stop execution if officerDetails is null
    }
